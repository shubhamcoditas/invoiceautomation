...
export function ServiceGroup() {
  ...
  const { data: serviceGroups = [], isLoading } = useQuery<ServiceGroup[]>({
  ...
  const handleOpenDialog = (serviceGroup?: ServiceGroup) => {
    if (serviceGroup) {
  ...
  // Remove duplicates based on service group name (case-insensitive)
  const uniqueServiceGroups = useMemo(() => {
    const seenNames = new Map<string, ServiceGroup>();
    
    serviceGroups.forEach((sg) => {
      const normalizedName = sg.name.trim().toLowerCase();
      const existing = seenNames.get(normalizedName);
      
      if (!existing) {
        seenNames.set(normalizedName, sg);
      } else {
        const existingDate = existing.createdAt ? new Date(existing.createdAt).getTime() : 0;
        const currentDate = sg.createdAt ? new Date(sg.createdAt).getTime() : 0;
      ...
      <Card>
        <CardHeader>
          <div className="flex items-center justify-between">
            <CardTitle>Service Group</CardTitle>
        ...
        <CardContent>
          ...
          {isLoading ? (
            ...
            <Table>
              ...
              <TableBody>
                {uniqueServiceGroups.map((serviceGroup) => (
                  <TableRow key={serviceGroup.id}>
                    <TableCell>{getVerticalName(serviceGroup.verticalId)}</TableCell>
                    <TableCell>
                    ...
                    <TableCell>
                      {serviceGroup.createdAt
                        ? new Date(serviceGroup.createdAt).toLocaleDateString()
                        : "-"}
                    </TableCell>
                    <TableCell className="text-right">
                      <div className="flex justify-end gap-2">
                        <Button
                          variant="ghost"
                          size="sm"
                          onClick={() => handleOpenDialog(serviceGroup)}
                        >
                        ...
                        <Button
                          variant="ghost"
                          size="sm"
                          onClick={() => handleDelete(serviceGroup.id)}
                        >
